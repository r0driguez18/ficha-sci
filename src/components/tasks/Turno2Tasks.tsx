  ...
  return (
    <div className="space-y-4 text-foreground">
      <div className="flex items-start space-x-2">
        ...
        <Label htmlFor="datacenter2" className="cursor-pointer text-foreground font-medium">Verificar Alarmes e Sistemas/Climatização DATA CENTER</Label>
      </div>
      
      <div className="flex items-start space-x-2">
        <Checkbox id="sistemas2" checked={tasks.sistemas} onCheckedChange={(c) => onTaskChange('sistemas', !!c)} />
        <Label htmlFor="sistemas2" className="cursor-pointer text-foreground">Verificar Sistemas: BCACV1 / BCACV2</Label>
      </div>

      <div className="flex items-start space-x-2">
        <Checkbox id="servicos2" checked={tasks.servicos} onCheckedChange={(c) => onTaskChange('servicos', !!c)} />
        <Label htmlFor="servicos2" className="cursor-pointer text-foreground">Verificar Serviços Vinti4/BCADirecto/Replicação/ Servidor MIA</Label>
      </div>
      ...
      <div className="flex items-start space-x-2">
        ...
        <Label htmlFor="verificarDebitos2" className="cursor-pointer text-foreground">Verificar Débitos/Créditos Aplicados no Turno Anterior</Label>
      </div>

      <div className="flex items-center space-x-2 ml-1">
        <Checkbox id="percurso76857_m2" checked={tasks.percurso76857} onCheckedChange={(c) => onTaskChange('percurso76857', !!c)} />
        <Label htmlFor="percurso76857_m2" className="cursor-pointer text-foreground font-medium mr-2">Percurso 76857 –</Label>
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-1">
            <Label htmlFor="p76857_14h" className="cursor-pointer text-foreground">14h</Label>
            <Checkbox id="p76857_14h" checked={tasks.percurso76857_14h} onCheckedChange={(c) => onTaskChange('percurso76857_14h', !!c)} />
          </div>
          <div className="flex items-center space-x-1">
            <Label htmlFor="p76857_16h" className="cursor-pointer text-foreground">16h</Label>
            <Checkbox id="p76857_16h" checked={tasks.percurso76857_16h} onCheckedChange={(c) => onTaskChange('percurso76857_16h', !!c)} />
          </div>
          <div className="flex items-center space-x-1">
            <Label htmlFor="p76857_19h" className="cursor-pointer text-foreground">19h</Label>
      ...
      <div className="flex items-start space-x-2">
        ...
        <Label htmlFor="inpsRetorno" className="cursor-pointer text-foreground">Processar e enviar os ficheiros retorno do INPS</Label>
      </div>

      <div className="flex items-start space-x-2">
        <Checkbox id="processarTef2" checked={tasks.processarTef} onCheckedChange={(c) => onTaskChange('processarTef', !!c)} />
        <Label htmlFor="processarTef2" className="cursor-pointer text-foreground font-medium">Processar Ficheiros TEF – RTR/RCT/ERR</Label>
      </div>

      <div className="flex items-start space-x-2">
        <Checkbox id="processarTelecomp2" checked={tasks.processarTelecomp} onCheckedChange={(c) => onTaskChange('processarTelecomp', !!c)} />
        <Label htmlFor="processarTelecomp2" className="cursor-pointer text-foreground font-medium">Processar Ficheiros Telecompensação – RCB/RTC/FCT/IMR</Label>
      </div>
      ...
      <div className="flex items-start space-x-2">
        ...
        <Label htmlFor="verificarPendentes" className="cursor-pointer text-foreground">Verificar Pendentes dos Balcões abertos</Label>
      </div>

      <div className="flex items-start space-x-2">
        <Checkbox id="validarSaco" checked={tasks.validarSaco} onCheckedChange={(c) => onTaskChange('validarSaco', !!c)} />
        <Label htmlFor="validarSaco" className="cursor-pointer text-foreground">Validar Contas Saco – Percurso 1935</Label>
      </div>

      <div className="flex items-start space-x-2">
        <Checkbox id="fecharBalcoes" checked={tasks.fecharBalcoes} onCheckedChange={(c) => onTaskChange('fecharBalcoes', !!c)} />
        <Label htmlFor="fecharBalcoes" className="cursor-pointer text-foreground">Fechar os Balcões Centrais</Label>
      </div>

      <div className="flex items-start space-x-2">
        <Checkbox id="verificarSistemas_fim" checked={tasks.verificarSistemas2} onCheckedChange={(c) => onTaskChange('verificarSistemas2', !!c)} />
        <Label htmlFor="verificarSistemas_fim" className="cursor-pointer text-foreground">Verificar Sistemas: <span className="font-bold">BCACV1 / BCACV2 / Replicação</span></Label>
      </div>

      <div className="pt-6 mt-4 border-t border-border">
        <Label htmlFor="observations2" className="text-sm font-semibold text-foreground uppercase tracking-wide">Outras Intervenções/Observações/Comunicações/Ocorrências</Label>
        <Textarea 
          id="observations2" 
          value={observations}
